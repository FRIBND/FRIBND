...
def validate_jp_field(filename):
    """
    Validate J-�� (spin-parity) field positioning in L-records.
    
    ENSDF Format Rule: J-�� field MUST be LEFT-JUSTIFIED starting at column 23.
    
    The J-�� field occupies columns 23-39 and contains spin-parity assignments like:
    - 3/2+ (firm assignment)
    ...
    CRITICAL: J-�� values must START at column 23 with NO leading spaces!
    """
    print(f"\nJ-�� FIELD VALIDATION: {filename}")
    print("=" * 60)
    print("Checking J-�� field positioning in columns 23-39...")
    print("ENSDF Rule: J-�� field values must be LEFT-JUSTIFIED starting at column 23")
    print()
    print('ENSDF 80-Column Ruler:')
    print('         1         2         3         4         5         6         7         8')
    print('12345678901234567890123456789012345678901234567890123456789012345678901234567890')
    print(' ' * 22 + '^----------------^ J-�� field (columns 23-39)')
    print()
    ...
    for line_num, line_content in enumerate(lines, 1):
        ...
        # Only check L-records for J-�� field validation
        if len(line_content) < 10 or ' L ' not in line_content[6:10]:
            continue
        ...
        if len(line_content) >= 23:
            ...
            if jp_field_stripped:
                ...
                if col22_char != ' ':
                    ...
                    print(f"[OK] Line {line_num:3d}: J-��='{jp_field_stripped}' correctly LEFT-JUSTIFIED at column 23")
                else:
                    # J-�� has leading space(s) - this is an error!
                    ...
                    print(f"[ERROR] Line {line_num:3d}: ERROR - J-��='{jp_field_stripped}' has leading space(s)")
    ...
    # Summary
    print(f"J-�� FIELD SUMMARY:")
    print(f"  Total J-�� fields analyzed: {jp_fields_analyzed}")
    print(f"  J-�� field positioning errors: {jp_field_errors}")
    print()
    
    if jp_field_errors == 0:
        print(f"[OK] SUCCESS: All J-�� fields correctly LEFT-JUSTIFIED at column 23")
        return True
    else:
        print(f"[ERROR] FAILED: {jp_field_errors} J-�� field positioning errors found")
        print(f"   CRITICAL: J-�� field values must be LEFT-JUSTIFIED starting at column 23")
        print(f"   Current violations: J-�� values have leading spaces instead of starting at column 23")
        return False
...
def validate_ri_field(filename):
    ...
    if ri_errors == 0:
        ...
        print("  Manually insert a space at column 22 for each error")
        print()
        return False
